...
class GetHighLow():
  ...
  def get_high_low_temp_db(
    self, coll_read, coll_write, find_key, sort_key, date_):
    """ Gets High and Low temp for the day. only between 11:30pm and 12pm """
    high_low = self.get_high_low_today(coll_read, find_key, sort_key, date_)
  ...
  def get_doc_today_db(self, col, key, date_):
    """ Gets Documents from today mongoDB database """
    today = self.date_request(date_)
  ...
  def get_high_low_today(self, col,find_key, sort_key,date_):
    today_doc = self.get_doc_today_db(col, find_key,date_)
